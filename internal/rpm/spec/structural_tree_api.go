...
func validateSectionRemoval(root *block, removeSet map[*block]bool) error {
	_, err := validatePostRemovalOwnership(root.Children, removeSet, false, false)
...
// active preceding section on that path. hasPrecedingSection records whether
// the current lexical scope has a non-preamble section before a conditional.
func validatePostRemovalOwnership(
	blocks []*block,
	removeSet map[*block]bool,
	needsSection, hasPrecedingSection bool,
) (bool, error) {
	for _, blk := range blocks {
		switch blk.Kind {
		case sectionBlock:
			needsSection = removeSet[blk]
			hasPrecedingSection = blk.Name != ""
		case rootBlock:
			var err error

			needsSection, err = validatePostRemovalOwnership(
				blk.Children,
				removeSet,
				needsSection,
				hasPrecedingSection,
		...
		case conditionalBlock:
			if hasPrecedingSection && containsBranchDirective(blk) && containsRemovedSection(blk, removeSet) {
				return false, fmt.Errorf("conditional block at %#q contains a branch directive across a removed section:\n%w",
					blk.Header, ErrConditionalSpansSections)
			}

			var err error

			needsSection, err = validateConditionalPostRemovalOwnership(
				blk,
				removeSet,
				needsSection,
				hasPrecedingSection,
...
func validateConditionalPostRemovalOwnership(
	...
	thenNeedsSection, err := validatePostRemovalOwnership(
	...
	elseNeedsSection, err := validatePostRemovalOwnership(
		conditional.Else,
		removeSet,
		needsSection,
		hasPrecedingSection,
