...
func validateConditionalRemoval(
	...
	if preceding != nil && preceding.Name != "" &&
		...
		return fmt.Errorf("conditional block at %#q contains a branch directive across a removed section:\n%w",
	...
	if wouldEmptySectionWrapper(child, removeSet) && containsBranchDirective(child) {
		return fmt.Errorf("conditional block at %#q contains branches that would be removed with its sections:\n%w",
			child.Header, ErrConditionalSpansSections)
	}
